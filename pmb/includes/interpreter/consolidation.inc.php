...
// +-------------------------------------------------+
// $Id: consolidation.inc.php,v 1.33.2.7 2016-09-29 13:39:22 jpermanne Exp $
...
function aff_facettes_multicritere($param,$parser){
	...
	global $opac_messages_search_facette;
	...
	if(isset($tab['multi_human_query'])) {
		...
		if(!isset($opac_messages_search_facette) && !is_array($opac_messages_search_facette)) {
			$opac_messages_search_facette = array();
			while(list($codelang, $libelle) = each($languages)) {
				...
				if (($charset != 'utf-8' and $codelang != 'ar') or ($charset == 'utf-8')) {
					...
					$messages->analyser();
					if(!in_array($messages->table['search_facette'], $opac_messages_search_facette)) {
						$opac_messages_search_facette[] = $messages->table['search_facette'];
					}
				}
			}
		}
		foreach ($opac_messages_search_facette as $opac_message_search_facette) {
			if(preg_match_all('`'.$opac_message_search_facette.' = \((.+?)\)`',$tmp,$out)){
				if(!$param[0]){
