...
// +-------------------------------------------------+
// $Id: bull_info.inc.php,v 1.60.2.4 2016-10-18 08:35:15 jpermanne Exp $
	...
	if(count($expl)){
		...
		foreach($expl as $exemplaire){
			...
			if(!$show_in_reception && (SESSrights & CATALOGAGE_AUTH)){
				$selector_prop = "toolbar=no, dependent=yes, resizable=yes, scrollbars=yes";
				$cart_click_expl = "onClick=\"openPopUp('./cart.php?object_type=EXPL&item=!!item!!', 'cart', 600, 700, -2, -2, '$selector_prop')\"";
				$cart_over_out = "onMouseOver=\"show_div_access_carts(event,".$exemplaire->expl_id.",'EXPL');\" onMouseOut=\"set_flag_info_div(false);\"";
				$cart_link = "<img src='./images/basket_small_20x20.gif' align='center' alt='middle' title=\"${msg[400]}\" $cart_click_expl $cart_over_out>";	
...
function show_bulletinage_info($bul_id, $lien_cart_ajout=1, $lien_cart_suppr=0, $flag_pointe=0, $lien_pointe=0 ) {
	...
	if ($bul_id) {
		if (SESSrights & CATALOGAGE_AUTH) {
			...
			if ($lien_cart_ajout) {
				$cart_link = "<img src='./images/basket_small_20x20.gif' align='middle' alt='basket' title=\"${msg[400]}\" $cart_click_bull $cart_over_out>";
