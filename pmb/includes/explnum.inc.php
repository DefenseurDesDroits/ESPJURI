...
// +-------------------------------------------------+
// $Id: explnum.inc.php,v 1.88.2.12 2017-05-11 10:17:41 arenou Exp $
...
function reduire_image ($userfile_name) {
	...
	if(file_exists("$base_path/temp/$userfile_name")){
		...
		$source_file = realpath($fichier_tmp).'[0]';
	}
	$error = true;
...
function construire_vignette($vignette_name='', $userfile_name='', $url='') {
	//On fait un truc un peut bizarre en 4.2, il faut prévoir un jolie hook en 5.0
	$contenu_vignette = "";
	if(function_exists("hook_construire_vignette")){
		$contenu_vignette = hook_construire_vignette($vignette_name, $userfile_name, $url);
		if($contenu_vignette!= ''){
			return $contenu_vignette;
		}
	}	
