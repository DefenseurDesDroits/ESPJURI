...
// +-------------------------------------------------+
// $Id: empr.inc.php,v 1.16.4.3 2017-02-15 08:13:44 jpermanne Exp $
...
switch ($action) {
	...
	case 'add_empr_limite':
		if ($sub_action=='add' && count($caddie)) {
			...
			$requete = "SELECT id_empr FROM empr where ((to_days(empr_date_expiration) - to_days(now()) ) <=  $pmb_relance_adhesion ) and empr_date_expiration >= now() $restrict_localisation $restrict_statut";
			...
			aff_paniers_empr(0, "./cart.php?object_type=$object_type&sub_action=add&empr_location_id=$empr_location_id&empr_statut_edit=$empr_statut_edit", "add_empr_limite",$msg["caddie_add_emprs"] , "", 0, 0, 0);
		}
		break;
	...
	case 'add_empr_categ_change':
		if ($sub_action=='add' && count($caddie)) {
			...
			$requete .=" where ((((age_min<> 0) || (age_max <> 0)) && (age_max >= age_min)) && (((DATE_FORMAT( curdate() , '%Y' )-empr_year) < age_min) || ((DATE_FORMAT( curdate() , '%Y' )-empr_year) > age_max))) $restrict_localisation";
			$res_rqt=pmb_mysql_query($requete);
			foreach ( $caddie as $id_caddie => $coche) {
       			if($coche){
       				$myCart = new empr_caddie($id_caddie);
			...
			// function aff_paniers_empr($item=0, $lien_origine="./circ.php?", $action_click = "add_item", $titre="", $restriction_panier="", $lien_edition=0, $lien_suppr=0, $lien_creation=1) {
			aff_paniers_empr(0, "./cart.php?object_type=$object_type&sub_action=add&empr_location_id=$empr_location_id", "add_empr_categ_change",$msg["caddie_add_emprs"] , "", 0, 0, 0);
		}
		break;
	default:
