...
// +-------------------------------------------------+
// $Id: series_list.inc.php,v 1.28.4.1 2016-09-02 12:51:02 jpermanne Exp $
...
if($nbr_lignes) {
	...
	if(!$user_input) {
		$requete = "SELECT * FROM series ORDER BY serie_name LIMIT $debut,$nb_per_page_gestion ";
		if ($last_param) $requete = "SELECT * FROM series ".$tri_param." ".$limit_param;
			...
			$requete="select *,".$members["select"]." as pert from series where ".$members["where"]." group by serie_id order by pert desc, serie_index limit $debut,$nb_per_page_gestion";
			}
	$res = @pmb_mysql_query($requete, $dbh);
	$parity=1;
