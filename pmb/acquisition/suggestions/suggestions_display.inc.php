...
// +-------------------------------------------------+
// $Id: suggestions_display.inc.php,v 1.35.2.6 2017-06-05 11:50:20 jpermanne Exp $
...
function show_list_sug($id_bibli=0) {
	...
	if($nbr_lignes) {
		...
		for($i=0;$i<$nbr;$i++) {
			...
			$img_pj = "<a href=\"$base_path/explnum_doc.php?explnumdoc_id=".$sug->get_explnum('id')."\" target=\"_blank\"><img src='$base_path/images/globe_orange.png' /></a>";
		...
		//Bouton Imprimer
		$url_print="./pdf.php?pdfdoc=listsug&user_input=".urlencode(stripslashes($user_input))."&statut=".$statut."&num_categ=".$num_categ."&sugg_location_id=".$sugg_location_id."&date_inf=".$date_inf."&date_sup=".$date_sup;
		if (is_array($user_id) && count($user_id) && is_array($user_statut) && count($user_statut)) {
			foreach($user_id as $k=>$v) {
				if ($user_id[$k] && (isset($user_statut[$k]))) {
