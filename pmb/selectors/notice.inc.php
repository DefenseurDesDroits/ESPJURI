...
// +-------------------------------------------------+
// $Id: notice.inc.php,v 1.29.4.2 2016-09-22 07:55:55 jpermanne Exp $
	...
	if($nbr_lignes) {
		...
		while(($notice=pmb_mysql_fetch_object($res))) {
			if($niveau_biblio){
				...
				print pmb_bidi("<div class='row'>
							<div class='left'>
								<a href='#' onclick=\"set_parent('$caller', '$notice->notice_id', '".trim(htmlentities(addslashes(strip_tags($display->result)),ENT_QUOTES,$charset)." ".($notice->code ? "($notice->code)" : ""))."','$callback')\">".$display->result."</a>
							</div>
							<div class='right'>
								".htmlentities($notice->code,ENT_QUOTES,$charset)."
							</div>
...
//Recherche par id de notice
$sel_search_form = str_replace("!!id_restrict!!", $id_restrict, $sel_search_form);
