...
// +-------------------------------------------------+
// $Id: authperso.inc.php,v 1.3.4.2 2016-10-27 12:55:39 ngantier Exp $
...
$base_url = "./select.php?what=authperso&authperso_id=$authperso_id&caller=$caller&p1=$p1&p2=$p2&p3=$p3&p4=$p4&p5=$p5&p6=$p6&no_display=$no_display&bt_ajouter=$bt_ajouter&dyn=$dyn&callback=$callback&infield=$infield"
		."&max_field=".$max_field."&field_id=".$field_id."&field_name_id=".$field_name_id."&add_field=".$add_field."&param1=$param1&param2=$param2&perso_id=$perso_id";
...
switch($action){
	...
	case 'update':
		...
		print $jscript;
		show_results($dbh, $user_input, $nbr_lignes, $page, $id);
		break;
	default:
		print $sel_header;
		$sel_search_form = str_replace("!!bouton_ajouter!!", $bouton_ajouter, $sel_search_form);
