...
// +-------------------------------------------------+
// $Id: pmbesConvertImport.class.php,v 1.8.4.2 2017-04-12 13:07:55 jpermanne Exp $
...
class pmbesConvertImport extends external_services_api_class {
	...
	function convert($notice, $convert_type_id, $import=0, $source_id=0) {
		...
		if (count($convert_type)) {
			$export= new convert(encoding_normalize::utf8_normalize($notice),$convert_type_id);
		...
		return array('notice'=>$notice, 'converted_notice'=>($charset!= "utf-8"?utf8_encode($this->converted_notice):$this->converted_notice));
	}
