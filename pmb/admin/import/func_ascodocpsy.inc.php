...
// +-------------------------------------------------+
// $Id: func_ascodocpsy.inc.php,v 1.6.4.8 2017-04-11 09:24:19 mbertin Exp $
...
function import_new_notice_suite() {
	global $dbh ;
	global $notice_id,$statutnot,$cree_expl_asco ;
	...
	if (count($info_911[0]) && $res && pmb_mysql_num_rows($res)) {
		$cp_id = pmb_mysql_result($res,0,0);
		$requete="insert into notices_custom_values (notices_custom_champ,notices_custom_origine,notices_custom_date) values($cp_id,$notice_id,'".$info_911[0]."')";
		pmb_mysql_query($requete);
	}
	...
	if ($info_914[0] && $res && pmb_mysql_num_rows($res)) {
		$cp_id = pmb_mysql_result($res,0,0);
		$requete="select max(notices_custom_list_value*1) from notices_custom_lists where notices_custom_champ=".$cp_id;
		$resultat=pmb_mysql_query($requete);
		$max=@pmb_mysql_result($resultat,0,0);
...
function genere_bulletin($perio_info,$bull_info,$isbull=true){
	...
	if(pmb_mysql_num_rows($res) == 0){
		...
		if(pmb_mysql_num_rows($res) > 1){//On tente d'affiner
			$search = "select bulletin_id from bulletins where bulletin_notice = $perio_id";
			if(strlen($bull_info['date']) == 10){
				$search.=" AND date_date= '".$bull_info['date']."'";
			}else{
				$search.=" AND mention_date LIKE '%".$bull_info['date']."%'";
			}
			$res = pmb_mysql_query($search);
