...
// +-------------------------------------------------+
// $Id: empr_list.inc.php,v 1.56.2.6 2017-01-25 15:25:15 jpermanne Exp $
...
switch ($sub) {
	...
	default :
		if ($form_cb) {
			$clause = "WHERE (empr_nom like '".str_replace("*", "%", $form_cb)."%' OR empr_cb like '".str_replace("*", "%", $form_cb)."%')" ;
		}
...
if ($nbr_lignes == 1) {
	...
	if (($empr_sort_rows)||($empr_show_rows)||($empr_filter_rows)) {
		...
		if (!$filter->error) {
			...
			$aff_filters.="<div id=\"el1Parent\" class=\"notice-parent\"><img src=\"./images/plus.gif\" class=\"img_plus\" name=\"imEx\" id=\"el1Img\" title=\"".$msg['admin_param_detail']."\" border=\"0\" onClick=\"expandBase('el1', true); return false;\">
		   							<b>".$msg["filters"]."</b></div>
