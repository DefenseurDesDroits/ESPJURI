...
// +-------------------------------------------------+
// $Id: transferts.inc.php,v 1.6.4.4 2017-02-28 13:36:47 jpermanne Exp $
...
switch($dest) {
	case "TABLEAU":
		...
		$worksheet->write_string(2,0,$msg["transferts_edition_tableau_titre"]);
		$worksheet->write_string(2,1,$msg["transferts_edition_tableau_section"]);
		$worksheet->write_string(2,2,$msg["transferts_edition_tableau_cote"]);
		$worksheet->write_string(2,3,$msg["transferts_edition_tableau_expl"]);
		$worksheet->write_string(2,4,$msg["transferts_edition_tableau_empr"]);
		$worksheet->write_string(2,5,$msg["transferts_edition_tableau_expl_owner"]);
		if ($site_origine==0 && $site_destination==0) {
			$worksheet->write_string(2,6,$msg["transferts_edition_tableau_source"]);
			$worksheet->write_string(2,7,$msg["transferts_edition_tableau_destination"]);
			$worksheet->write_string(2,8,$msg["transferts_edition_tableau_motif"]);
		}elseif ($site_origine==0) {
			$worksheet->write_string(2,6,$msg["transferts_edition_tableau_source"]);
			$worksheet->write_string(2,7,$msg["transferts_edition_tableau_motif"]);
		}elseif ($site_destination==0) {
			$worksheet->write_string(2,6,$msg["transferts_edition_tableau_destination"]);
			$worksheet->write_string(2,7,$msg["transferts_edition_tableau_motif"]);
		}else{
			$worksheet->write_string(2,6,$msg["transferts_edition_tableau_motif"]);
		}

		$ligne=3;
		while ($value = pmb_mysql_fetch_array($req)) {
			$worksheet->write_string($ligne,0,strip_tags(aff_titre($value[0], $value[1])));
			$worksheet->write_string($ligne,1,$value[4]);
			$worksheet->write_string($ligne,2,$value[3]);
			$worksheet->write_string($ligne,3,$value[2]);
			$worksheet->write_string($ligne,4,$value[10]);
			$worksheet->write_string($ligne,5,$value[7]);
			if ($site_origine==0 && $site_destination==0) {
				$worksheet->write_string($ligne,6,$value[6]);
				$worksheet->write_string($ligne,7,$value[5]);
				$worksheet->write_string($ligne,8,$value[8]);
			}elseif ($site_origine==0) {
				$worksheet->write_string(2,6,$msg["transferts_edition_tableau_source"]);
				$worksheet->write_string($ligne,6,$value[6]);
				$worksheet->write_string($ligne,7,$value[8]);
			}elseif ($site_destination==0) {
				$worksheet->write_string(2,6,$msg["transferts_edition_tableau_destination"]);
				$worksheet->write_string($ligne,6,$value[5]);
				$worksheet->write_string($ligne,7,$value[8]);
			}else{
				$worksheet->write_string($ligne,6,$value[8]);
			}
