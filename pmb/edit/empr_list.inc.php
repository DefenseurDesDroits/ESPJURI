...
// +-------------------------------------------------+
// $Id: empr_list.inc.php,v 1.36.2.3 2017-02-22 11:14:10 jpermanne Exp $
...
if($nbr_lignes) {
	...
	switch($dest) {
		...
		default:
			...
			if ($empr_show_caddie) $bt_add_panier="&nbsp;&nbsp;<input type='button' class='bouton_small' value='".$msg["add_empr_cart"]."' onClick=\"openPopUp('./cart.php?object_type=EMPR&action=add_empr_$sub&empr_location_id=$empr_location_id&empr_statut_edit=$empr_statut_edit', 'cart', 600, 700, -2, -2,'$selector_prop_ajout_caddie_empr'); return false;\">";
			else $bt_add_panier="";
			echo "
			...
			if ($sub=="limite" || $sub=="depasse") echo "<input type='button' class='bouton_small' value='".htmlentities($msg["print_all_relances"],ENT_QUOTES,$charset)."' onclick='document.location=\"./edit.php?categ=$categ&sub=$sub&action=print_all&empr_location_id=$empr_location_id&empr_statut_edit=$empr_statut_edit\"'>&nbsp;";
