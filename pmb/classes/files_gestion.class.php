...
// +-------------------------------------------------+
// $Id: files_gestion.class.php,v 1.3.4.1 2015-12-21 14:43:45 jpermanne Exp $
...
class files_gestion {	
	...
	function get_list($post_url="admin.php?categ=mailtpl&sub=img") {
		...
		foreach($this->info as $elt){
			...
			$tpl_elt=str_replace('!!vignette!!',"<img height='15' width='15' src=\"".$this->url.$elt['name']."\" alt=\"\" onmouseover=\"show_div_img(event,'".$this->url.$elt['name']."')\" onmouseout=\"hide_div_img()\" />", $tpl_elt);	
			$tpl_list.=$tpl_elt;	
		}
