...
// +-------------------------------------------------+
// $Id: cms_build.class.php,v 1.43.4.6 2016-11-22 08:05:19 ngantier Exp $
...
*/
	...
	function get_clean_cache_button(){
		global $msg,$base_path;
		
		$rqt = "SELECT * FROM cms_cache_cadres";
		$res = pmb_mysql_query($rqt,$dbh);
		if(pmb_mysql_num_rows($res)){
			$clean_cache_button ='<div data-dojo-type=\'dijit/form/Button\' data-dojo-props=\'id:"clean_cache_button",onclick:"if(confirm(\"'.$msg['cms_clean_cache_confirm'].'\")){document.location=\"'.$base_path.'/cms.php?categ=build&sub=block&action=clean_cache\";}"\'>'.$msg['cms_clean_cache'].'</div>';
		}else{
