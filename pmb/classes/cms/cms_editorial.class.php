...
// +-------------------------------------------------+
// $Id: cms_editorial.class.php,v 1.43.2.8 2017-02-14 08:28:02 jpermanne Exp $
			...
			if($datatype=="all") {
				...
				if(count($tab_pp)){
					foreach ( $tab_pp as $id ) {
       					$req_del="delete from cms_editorial_words_global_index where num_obj='".$this->id."' and type = '".$this->type."' and code_champ='".($id*1)."' ";
       					pmb_mysql_query($req_del,$dbh);
						//la table pour les recherche exacte
						$req_del="delete from cms_editorial_fields_global_index where num_obj='".$this->id."' and type = '".$this->type."' and code_champ='".($id*1)."' ";
						pmb_mysql_query($req_del,$dbh);	
					}
				}
			}
