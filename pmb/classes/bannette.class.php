...
// +-------------------------------------------------+
// $Id: bannette.class.php,v 1.152.2.18 2017-02-17 13:24:44 jpermanne Exp $
...
class bannette {
	...
	var $bannette_aff_notice_number = true; //Afficher le nombre de notices envoyées dans le mail
	...
	function getData() {
		...
		if (!$this->id_bannette) {
			...
			$this->bannette_aff_notice_number=true;
		} else {
	...
	function update($temp) {
		...
		$req.="bannette_aff_notice_number='$temp->bannette_aff_notice_number',";
	...
	function construit_diff() {
		...
		if($this->document_generate){
			$this->document_diffuse = "<html><head><META HTTP-EQUIV='CONTENT-TYPE' CONTENT='text/html; charset=".$charset."'>$css</head><body>".$titre . $this->aff_document . $footer. "</body></html>";
		}
	...
	function diffuser() {
		...
		//on va remplacer les messages dans la langue de l'emprunteur
		$tmp_msg = $msg;
		$array_msg = array($lang=>$msg);
		$requete_list_empr = "select id_empr, empr_lang from empr, bannette_abon, bannettes where num_bannette='".$this->id_bannette."' and num_empr=id_empr and num_bannette=id_bannette order by empr_nom, empr_prenom ";
		$res_list_empr = pmb_mysql_query($requete_list_empr, $dbh);
		while ($list_empr=pmb_mysql_fetch_object($res_list_empr)) {
			if (!isset($array_msg[$list_empr->empr_lang])) {
				$messages = new XMLlist($include_path."/messages/".$list_empr->empr_lang.".xml", 0);
				$messages->analyser();
				$array_msg[$list_empr->empr_lang] = $messages->table;
			}
			//on utilise la langue de l'emprunteur
			$msg = $array_msg[$list_empr->empr_lang];
			...
			$requete .= "where num_bannette='".$this->id_bannette."' and id_empr=".$list_empr->id_empr." and num_empr=id_empr and empr_statut=idstatut and num_bannette=id_bannette ";
			$res = pmb_mysql_query($requete, $dbh);
			...
			//remplacements
			$texte = str_replace("!!print_n_notices!!", sprintf($msg["print_n_notices"],$this->nb_notices), $texte);
			if (($this->nb_notices_diff >= $this->nb_notices) || (!$this->nb_notices_diff)) {
				$nb_envoyees = $this->nb_notices ;
			} else {
				$nb_envoyees = $this->nb_notices_diff;
			}
			$texte = str_replace("!!dsi_diff_n_notices!!", sprintf($msg["dsi_diff_n_notices"],$nb_envoyees,$this->nb_notices), $texte);
		...
		//on remet la langue d'origine de l'utilisateur
		$msg = $tmp_msg;
		// remplissage du panier avec le contenu de la bannette
		if ($this->num_panier) {
			$temp_requete = "delete from caddie_content where caddie_id='".$this->num_panier."'" ;
	...
	function construit_contenu_HTML ($use_limit=1) {
		...
		if($this->bannette_aff_notice_number){
			$resultat_aff .= "<span class=\"dsi_hide_for_emails\"><hr />!!dsi_diff_n_notices!!<hr /></span>";
		}
		if($this->notice_tpl){
				...
				if($this->document_generate){				
					...
					if(!$tpl_document) {
						...
						if ($n->niveau_biblio == 'm'|| $n->niveau_biblio == 'b') {
							$mono=new mono_display($n,$environement["short"],"",$environement["ex"],"","","",0,1,$environement["exnum"],0,"",0,true,false,0,0,1,$this->id_bannette);
							...
							$serial = new serial_display($n, 6, "", "", "", "", "", 0,1,$environement["exnum"],0, false,0,0,'',false,1,$this->id_bannette);
	...
	function construit_liens_HTML() {
		...
		$entete = $this->replace_equation($entete) ;
