...
// +-------------------------------------------------+
// $Id: authperso.class.php,v 1.12.2.2 2017-02-06 13:57:38 jpermanne Exp $
...
class authperso {
	...
	function get_list_selector($id_to_view=0) {
		global $msg,$charset,$dbh;
		global $user_query, $user_input,$page,$nbr_lignes,$last_param;		
		global $callback;
		global $caller;		
		...
		if($id_to_view){
			$isbd=$this->get_isbd($id_to_view);
			return "<br /><a href='#' onclick=\"set_parent('$caller', '$id_to_view', '".htmlentities(addslashes(str_replace("\r"," ",str_replace("\n"," ",$isbd))),ENT_QUOTES, $charset)."','$callback')\">".
		...
		while(($r=pmb_mysql_fetch_object($res))) {
			$id=$r->id_authperso_authority;
			$isbd=$this->get_isbd($id);
			$auth_lines.="<a href='#' onclick=\"set_parent('$caller', '$id', '".htmlentities(addslashes(str_replace("\r"," ",str_replace("\n"," ",$isbd))),ENT_QUOTES, $charset)."','$callback')\">".
					htmlentities($isbd,ENT_QUOTES, $charset)."</a><br />";			
		}
