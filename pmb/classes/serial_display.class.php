...
// +-------------------------------------------------+
// $Id: serial_display.class.php,v 1.161.2.12 2017-06-08 06:29:50 jpermanne Exp $
...
class serial_display {
	...
	var $header_texte	= '';			// chaine accueillant le chapeau de notice sans html
	...
	var $show_map=1;
	var $context_dsi_id_bannette=0;
	var $show_abo_actif = 0;
...
//				if($lien_suppr_cart)$param['lien_suppr_cart']=$lien_suppr_cart;
	...
	function do_isbd() {
		...
		if (!$this->no_link) {
			$requete="select linked_notice as notice_liee_id, relation_type, rank, l.niveau_biblio as lnb, l.niveau_hierar as lnh from notices_relations, notices as l where num_notice=".$this->notice_id." and linked_notice=l.notice_id order by relation_type,rank";
			...
			if (pmb_mysql_num_rows($result_linked)) {
				$this->display_linked_notices($result_linked,'up');
			}
		}
		
		//Impression : Liste des relations horizontales filles (les mères sont déjà affichées avec les notices parentes)
		//on prépare les relations horizontales
		global $liste_type_relation_up;
		if (!$liste_type_relation_up) $liste_type_relation_up=new marc_list("relationtypeup");
		global $liste_type_relation_down;
		if (!$liste_type_relation_down) $liste_type_relation_down=new marc_list("relationtypedown");
		...
		if ((!$this->no_link) && ($this->print_mode) && (count($corresp_relation_up_down))) {
			$requete="select num_notice as notice_liee_id, relation_type, rank, l.niveau_biblio as lnb, l.niveau_hierar as lnh from notices_relations, notices as l";
			$requete.=" where linked_notice=".$this->notice_id." and num_notice=l.notice_id and relation_type IN (".implode(",",$corresp_relation_up_down).")";
			$requete.=" order by relation_type,rank";
		
			$result_linked=pmb_mysql_query($requete) or die(pmb_mysql_error());
			//Si il y en a, on prépare l'affichage
			if (pmb_mysql_num_rows($result_linked)) {
				$this->display_linked_notices($result_linked,'down');
			}
		}
...
function display_linked_notices($result_linked,$sens){
	global $base_path;

	//on prépare les relations montantes
	global $relation_typeup ;
	if (!$relation_typeup) $relation_typeup=new marc_list("relationtypeup");
	//on prépare les relations descendantes
	global $relation_typedown;
	if (!$relation_typedown) $relation_typedown=new marc_list("relationtypedown");

	//On définit le tableau à utiliser
	$relations = ${'relation_type'.$sens};
	...
	while ($r_rel=pmb_mysql_fetch_object($result_linked)) {
		...
		if ($r_rel->lnb=='s' && $r_rel->lnh=='1') {
			...
			$link_serial_sub = $base_path."/catalog.php?categ=serials&sub=view&serial_id=".$r_rel->notice_liee_id;
			...
			$serial = new serial_display($r_rel->notice_liee_id, 0, $link_serial_sub, $link_analysis, $link_bulletin, "", "", 0, $this->print_mode, $this->show_explnum, $this->show_statut, $this->show_opac_hidden_fields, 1, true);
			$aff = $serial->header;
			...
			$serial = new serial_display($r_rel->notice_liee_id, 0, $link_serial, $link_analysis, $link_bulletin, "", "", 0, $this->print_mode, $this->show_explnum, $this->show_statut, $this->show_opac_hidden_fields, 1, true);
			$aff = $serial->result;
		} else {
			if($link_parent && $r_rel->lnb=='b' && $r_rel->lnh=='2'){
				$requete="SELECT bulletin_id FROM bulletins WHERE num_notice='".$r_rel->notice_liee_id."'";
			...
			// dans les autres cas
			$parent_notice=new mono_display($r_rel->notice_liee_id,0,$link_parent, 1, '', "", '', 0, $this->print_mode, $this->show_explnum, $this->show_statut, '', 1, true, $this->show_opac_hidden_fields, 0);
		...
		if (pmb_mysql_num_rows($result_linked)==1) {
			$this->isbd.="<br /><b>".$relations->table[$r_rel->relation_type]."</b> ".$aff."<br />";
		} else {
			if ($r_rel->relation_type!=$r_type_local) {
				$r_type_local=$r_rel->relation_type;
				if ($ul_opened) {
					$this->isbd.="</ul>";
					$this->isbd.="\n<b>".$relations->table[$r_rel->relation_type]."</b>";
					$this->isbd.="\n<ul class='notice_rel'>\n";
					$ul_opened=true;
				} else {
					$this->isbd.="\n<br />";
	...
	if ($this->notice->code || $this->notice->thumbnail_url) {
		if ($pmb_book_pics_show=='1' && ($pmb_book_pics_url || $this->notice->thumbnail_url)) {
			...
			if (($this->notice->thumbnail_url) && (preg_match('`^(.+)?getimage\.php`',$this->notice->thumbnail_url,$out))) {
				...
				$url_image = $prefix_url_image."getimage.php?url_image=".urlencode($pmb_book_pics_url)."&amp;noticecode=!!noticecode!!&amp;vigurl=".urlencode($this->notice->thumbnail_url) ;
			}
			if ($depliable) {
...
// 		}
		...
		if ($this->show_abo_actif && $this->icon_abo_actif) $this->header = $this->icon_abo_actif." ".$this->header;
