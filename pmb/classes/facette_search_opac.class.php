...
// +-------------------------------------------------+
// $Id: facette_search_opac.class.php,v 1.17.4.4 2017-04-25 13:10:34 jpermanne Exp $
...
class facette_search {
	...
	public function get_display_list(){
		...
		while($row = pmb_mysql_fetch_object($result)){
			...
			$td_javascript="
				onMouseDown=\"document.location='./admin.php?categ=opac&sub=facette_search_opac&section=facette&action=edit&id=".$row->id_facette."'\"
        		onmouseover=\"this.className='surbrillance'\" onmouseout=\"this.className='".$pair_impair."'\" 
        		style=\"cursor : pointer\" ";			
			$lst .= "
				<tr class='".$pair_impair."'>
					<td>
						<input type='button' class='bouton_small' value='-' onClick=\"document.location='./admin.php?categ=opac&sub=facette_search_opac&section=facette&action=up&id=".$row->id_facette."'\"/></a>
						<input type='button' class='bouton_small' value='+' onClick=\"document.location='./admin.php?categ=opac&sub=facette_search_opac&section=facette&action=down&id=".$row->id_facette."'\"/>
					</td>
	...
	public static function facette_up($id){
		$requete="select facette_order from facettes where id_facette=$id";
		...
		if ($ordre_max) {
			...
			$requete="update facettes set facette_order='".$ordre_max."' where id_facette=$id";
	...
	public static function facette_down($id){
		...
		if ($ordre_min) {
			...
			$requete="update facettes set facette_order='".$ordre_min."' where id_facette=$id";
			pmb_mysql_query($requete);
	...
	public static function facette_order_by_name(){
		$query = "SELECT id_facette  FROM facettes order by facette_name";
		$result = pmb_mysql_query($query);	
		$i=1;
		while($row = pmb_mysql_fetch_object($result)){
			pmb_mysql_query("UPDATE facettes SET facette_order='".$i."' where id_facette=".$row->id_facette);
			$i++;
		}
	}
