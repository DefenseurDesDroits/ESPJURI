...
// +-------------------------------------------------+
// $Id: mono_display_unimarc.class.php,v 1.39.4.3 2017-05-15 12:50:55 jpermanne Exp $
...
function do_isbd() {
	...
	if($this->notice->code) {
		if(isISBN($this->notice->code)) {
			if ($zoneNote) { $zoneNote .= '.&nbsp;-&nbsp;'.$msg['isbd_notice_isbn'].' '; } else { $zoneNote = $msg['isbd_notice_isbn'].' ';}
			} else {
...
function do_image(&$entree) {
	...
	if ($this->notice->code || $this->notice->thumbnail_url) {
		if ($pmb_book_pics_show=='1' && ($pmb_book_pics_url || $this->notice->thumbnail_url)) {
			...
			if ($depliable) {
				$image = "<img id='PMBimagecover".$this->notice_id."' src='".$prefix_url_image."images/vide.png' align='right' hspace='4' vspace='2' isbn='".$code_chiffre."' url_image='".$url_image."'>";
			} else {
