...
// +-------------------------------------------------+
// $Id: rss_flux.class.php,v 1.31.4.4 2017-05-04 07:53:09 jpermanne Exp $
...
// ---------------------------------------------------------------
	...
	function items_notices() {
		...
		if ($this->nb_bannettes) {
			$rqt[] = "select distinct notice_id, index_sew, create_date, update_date, index_serie, tnvol, year, date_parution 
		...
		$query_not = "select distinct notice_id, index_sew, DATE_FORMAT(create_date,'%a, %e %b %Y %T') as pubdate from tmpfluxrss order by $opac_flux_rss_notices_order" ;
