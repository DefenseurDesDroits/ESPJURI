...
// +-------------------------------------------------+
// $Id: cms_article.class.php,v 1.18.2.3 2016-09-21 07:58:52 apetithomme Exp $
...
class cms_article extends cms_editorial {
	...
	public function duplicate($num_parent = 0) {
		...
		//au tour des descripteurs...
		for($i=0 ; $i<count($this->descriptors) ; $i++){
			$rqt = "insert into cms_articles_descriptors set num_article = '".($id*1)."', num_noeud = '".$this->descriptors[$i]."',article_descriptor_order='".$i."'";
			pmb_mysql_query($rqt,$dbh);
		}
