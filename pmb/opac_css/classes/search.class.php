...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.148.2.18 2017-05-16 08:57:20 ngantier Exp $
...
class search {
	...
	function get_field($i,$n,$search,$pp) {
		...
		if ($s[0]=="f") {
					...
					if ($visibility) {
						...
						switch ($input["TYPE"]) {
							...
							case "hidden":
								...
								$r.="<input type='hidden' id=\"fieldvar_".$n."_".$search."[".$varname."][]\" name=\"fieldvar_".$n."_".$search."[".$varname."][]\" value=\"".htmlentities($hidden_value,ENT_QUOTES,$charset)."\"/>";
								if ($ff["VAR"][$j]["OPTIONS"]["INPUT"][0]["CLASS"]) {
									$r.="</span>";
								}
								break;
			...
			if ($$op == "AUTHORITY"){
				$libelle = "";
				if($v[0]!= 0){
					$aut=new authperso($v[0]);
					$libelle = $aut->get_isbd($v[0]);
				}
				$r.="<span class='search_value'><input autfield='$fname_id' onkeyup='fieldChanged(\"$fnamesans\",this.value,event);' callback='authoritySelected' completion='$ajax' $fnamevar_id id='$fnamesanslib' name='$fnamelib' value='".htmlentities($libelle,ENT_QUOTES,$charset)."' type='text' class='".($fieldvar['authority_id'][0] ? "authorities " : "")."ext_search_txt' /></span>
								";
			}else{
	...
	function show_form($url,$result_url,$result_target='') {
		...
		if ($opac_extended_search_auto) $r="<select name='add_field' id='add_field' onChange=\"if (this.form.add_field.value!='') { this.form.action='$url'; this.form.target=''; if(this.form.launch_search)this.form.launch_search.value=0; this.form.submit();} else { alert('".htmlentities($msg["multi_select_champ"],ENT_QUOTES,$charset)."'); }\" >\n";
			...
			foreach ( $this->dynamicfields as $key => $value ) {
				if(!$this->pp[$key]->no_special_fields && count($this->pp[$key]->t_fields) && ($key != 'a')){
					...
					if (count($array_dyn_tmp)) {
						...
						$r_custom.=implode('',$array_dyn_tmp);
					}
					$r_custom.="</optgroup>\n";
					$open_optgroup = 0;
				}
	...
	function parse_search_file($fichier_xml) {
		...
		$tempFile = $base_path."/temp/XML".preg_replace("/[^a-z0-9]/i","",$fileInfo['dirname'].$fileInfo['filename'].$lang.$charset).".tmp";
		...
		if ($dejaParse) {
			...
			if(count($cache) == 10){
				...
				$this->dynamicfields_order = $cache[6];
				$this->dynamicfields_hidebycustomname = $cache[7];
				$this->specialfields = $cache[8];
				$this->tableau_speciaux = $cache[9];
			}else{
				//SOUCIS de cache...
				unlink($tempFile);
				$this->parse_search_file($fichier_xml);
	...
	public function serialize_search($recurse_history=false) {
		global $search;
