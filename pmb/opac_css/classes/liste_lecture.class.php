...
// +-------------------------------------------------+
// $Id: liste_lecture.class.php,v 1.23.4.3 2016-01-28 15:39:52 dgoron Exp $
...
class liste_lecture {
		...
		if(!$this->id_liste){
			...
			$liste_gestion = str_replace('!!inscrit_list!!','',$liste_gestion);	
				...
				while(($empr=pmb_mysql_fetch_object($res))){
					if($empr->confidential) $aff_empr .= "<img border=0 align='top' src='".get_url_icon('cross.png', 1)."'  onclick=\"delete_from_liste('".$this->id_liste."','".$empr->id_empr."');\">";
	...
	function aff_navigation_notices($notices=array(),$id_liste, $sub){
		...
		if($precedente > 1) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=1\"><img src=\"".get_url_icon('first.gif')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[start]\"";
			$affichage .= " title=\"$msg[first_page]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('first-grey.gif')."\">";
		}
	
		if($precedente > 0) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=$precedente\"><img src=\"".get_url_icon('prev.gif')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[prec]\"";
			$affichage .= " title=\"$msg[prec]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('prev-grey.gif')."\">";
		}
		...
		if($suivante<=$nbepages) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=$suivante\"><img src=\"".get_url_icon('next.gif')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[next]\"";
			$affichage .= " title=\"$msg[next]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('next-grey.gif')."\">";
		}
	
		// affichage du lien vers la fin
		if($suivante < $nbepages) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=$nbepages\"><img src=\"".get_url_icon('last.gif')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[end]\"";
			$affichage .= " title=\"$msg[end]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('last-grey.gif')."\">";
		}
