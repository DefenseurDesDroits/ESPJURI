...
// +-------------------------------------------------+
// $Id: notice_info.class.php,v 1.39.2.11 2016-08-26 10:07:40 dbellamy Exp $
...
class notice_info {
	...
	function fetch_data() {
		...
		if(isset($_SESSION["cart"]) && in_array($this->notice_id, $_SESSION["cart"])) {
			if ($use_opac_url_base)	$this->memo_iconcart="<span id='baskets".$this->notice_id."'><a href='#' class=\"img_basket_exist\" title=\"".$msg['notice_title_basket_exist']."\"><img src=\"".get_url_icon('basket_exist.png', 1)."\" border=\"0\" alt=\"".$msg['notice_title_basket_exist']."\" /></a></span>";
			else $this->memo_iconcart="<span id='baskets".$this->notice_id."'><a href='#' class=\"img_basket_exist\" title=\"".$msg['notice_title_basket_exist']."\"><img src=\"".get_url_icon('basket_exist.png')."\" border=\"0\" alt=\"".$msg['notice_title_basket_exist']."\" /></a></span>";
		} else {
			...
			if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $this->notice->thumbnail_url)) {
				...
				if ($this->notice->thumbnail_url) {
					if(strpos($this->notice->thumbnail_url,'data:image',0)===0) {
						...
						$url_image_ok = $opac_url_base.'getimage.php?url_image='.rawurlencode($this->notice->thumbnail_url);
					}
					$title_image_ok="";
				} else {
