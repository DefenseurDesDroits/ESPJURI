...
// +-------------------------------------------------+
// $Id: demandes.class.php,v 1.31.2.8 2017-03-27 14:53:13 dgoron Exp $
...
// 				}
				...
				if($demandes_notice_auto_tpl){
					if(self::is_notice_visible($dmde)){
						$link_noti = "<td class='empr_demandes_col_linked'><a href='".$opac_url_base."index.php?lvl=notice_display&id=".$dmde->num_notice."' alt='".$msg['demandes_see_notice']."' title='".$msg['demandes_see_notice']."'><img src='$base_path/images/mois.gif' /></a></td>";
					} else{
				...
				$liste .= "<td class='empr_demandes_col1'><img hspace=\"3\" border=\"0\" onclick=\"expand_action('action".$dmde->id_demande."','$dmde->id_demande', true); return false;\" title=\"\" id=\"action".$dmde->id_demande."Img\" name=\"imEx\" class=\"img_plus\" src=\"./images/plus.gif\"></td>";
				...
				if(!$opac_demandes_affichage_simplifie) {
					...
					for ($i=0; $i<count($perso_["FIELDS"]); $i++) {
						$p=$perso_["FIELDS"][$i];
						if($p["OPAC_SHOW"]) $liste .= "<td class='empr_demandes_col_".$p["NAME"]."'>".nl2br($p["AFF"])."</td>";
					}
					$liste .= $link_noti;
				} else {
					$liste .="
	...
	*/
	...
	function show_consult_form($last_modified=0){
		...
		global $pmb_type_audit,$opac_demandes_no_action,$base_path;
