...
// +-------------------------------------------------+
// $Id: record_datas.class.php,v 1.18.2.24 2017-04-26 12:52:18 jpermanne Exp $
...
class record_datas {
	...
	*/
	...
	public function __construct($id) {
		...
		global $to_print;
		global $opac_avis_allow, $opac_allow_add_tag, $opac_show_suggest_notice, $opac_shared_lists;
	...
	public function get_p_perso() {
		if (!$this->p_perso) {
			...
			if((is_array($ppersos['FIELDS'])) && (count($ppersos['FIELDS']))){
			...
			if ($result && pmb_mysql_num_rows($result)) {
				if ($avis = pmb_mysql_fetch_object($result)) {
					...
					if ($avis->qte) {
						...
						$query = "select note, commentaire, sujet, DATE_FORMAT(dateajout,'".$msg['format_date']."') as date_avis, empr_login, empr_nom, empr_prenom 
									from avis left join empr on id_empr=num_empr 
									where num_notice='$this->id' and valide=1 order by avis_rank, note desc, id_avis desc";
						$result = pmb_mysql_query($query, $dbh);
						if ($result && pmb_mysql_num_rows($result)) {
							while ($avis = pmb_mysql_fetch_object($result)) {
								$this->avis['avis'][] = array(
										...
										'empr_login' => $avis->empr_login,
										'empr_nom' => $avis->empr_nom,
										'empr_prenom' => $avis->empr_prenom
								);
	...
	public function get_availability() {
		if (!$this->availability) {
			...
			if (count($expls_datas['expls'])) {
				...
				// Pas d'exemplaires
				if($opac_show_empty_items_block) {
					$availability = "empty";
				} else {
					$availability = "none";
				}
		...
		if (!$this->picture_url && ($this->get_code() || $this->get_thumbnail_url())) {
			...
			if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $this->get_thumbnail_url())) {
				if ($this->get_thumbnail_url()) {					
					$this->picture_url = $opac_url_base."getimage.php?url_image=".urlencode($this->get_thumbnail_url());
				} else {
	...
	public function get_picture_title() {
		...
		if (!$this->picture_title && ($this->get_code() || $this->get_thumbnail_url())) {
			global $charset;
			global $opac_show_book_pics, $opac_book_pics_url, $opac_book_pics_msg;
			if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $this->get_thumbnail_url())) {
				if ($this->get_thumbnail_url()) {
					$this->picture_title = htmlentities($this->get_tit1(), ENT_QUOTES, $charset);
				} else {
					$this->picture_title = htmlentities($opac_book_pics_msg, ENT_QUOTES, $charset);
				}
			}
		}
		...
		if (!isset($this->relations_up)) {
			...
			if ($result && pmb_mysql_num_rows($result)) {
				while ($parent = pmb_mysql_fetch_object($result)) {
					...
					$this->relations_up[$parent->relation_type]['relation_type'] = $parent->relation_type;
				}
			}
		}
		...
		if (!isset($this->relations_down)) {
			...
			if ($result && pmb_mysql_num_rows($result)) {
				while ($parent = pmb_mysql_fetch_object($result)) {
					...
					$this->relations_down[$parent->relation_type]['relation_type'] = $parent->relation_type;
				}
			}
		}
