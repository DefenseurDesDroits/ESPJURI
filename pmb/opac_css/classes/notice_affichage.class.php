...
// +-------------------------------------------------+
// $Id: notice_affichage.class.php,v 1.415.2.36 2017-05-22 11:51:56 jpermanne Exp $
...
class notice_affichage {
	...
	var $parents_header_without_html = false;
	
	protected $onglet_perso;

	// constructeur------------------------------------------------------------
	function notice_affichage($id, $liens="", $cart=0, $to_print=0,$header_only=0,$no_header=0, $show_map=1, $parents_header_without_html = false) {
	...
	function fetch_data() {
		...
		if(!$this->header_only) {
			...
			if ($this->notice->niveau_biblio =='b') {
				...
				$requete.= "where linked_notice='".$this->notice_id."' $statut_r order by relation_type, rank,create_date";
			}
	...
	function get_authperso_display(){
		...
		foreach($this->authperso_info as $auth_num => $auth){
			if($authperso_name!=$auth['authperso_name']){
				...
				$aff.="<tr><td class='bg-grey' align='right'><span class='etiq_champ'>".$authperso_name." : </span><td class='public_line_value'>";
				$new=1;
			}
			if(!$new)	$aff.=", ";
			$aff.=$auth['auth_see'];
	...
	function do_header($id_tpl=0) {
		...
		if ($this->notice->lien) {
			...
			$this->notice_header_doclink .= "&nbsp;<span class='notice_link'><a href=\"".$this->notice->lien."\" target=\"_blank\" type='external_url_notice'>";
			...
			if ($explnumscount == 1) {
				...
				if ($opac_visionneuse_allow && $this->docnum_allowed && ($allowed_mimetype && in_array($expl->explnum_mimetype,$allowed_mimetype))){
					...
					<a href='#' onclick=\"auth_popup('./ajax.php?module=ajax&categ=auth&new_tab=1&callback_url=".rawurlencode($opac_url_base."doc_num.php?explnum_id=".$explnumrow->explnum_id)."')\" alt='$alt' title='$alt'>";
						...
						$this->notice_header_doclink .= "
					<a href=\"./doc_num.php?explnum_id=".$explnumrow->explnum_id."\" target=\"_blank\">";
					}
				}
	...
	function do_parents() {
		...
		$requete="select linked_notice, relation_type, rank from notices_relations join notices on notice_id=linked_notice $acces_j $statut_j
				...
				order by relation_type,rank,create_date";
...
//			}
			...
			if (pmb_mysql_num_rows($result_linked)==1) {
				...
				if ($this->parents_header_without_html) {
					$this->parents.=$memo_notice[$r_rel->linked_notice]["header_without_html"];
				} else {
	...
	function aff_resa_expl() {
		...
		if($this->notice->niveau_biblio != 's') {
			...
			if ( (is_null($this->dom_2) && $opac_show_exemplaires && $this->visu_expl && (!$this->visu_expl_abon || ($this->visu_expl_abon && $_SESSION["user_code"]))) || ($this->rights & 8) ) {
				...
				$temp = static::expl_list($this->notice->niveau_biblio,$this->notice_id, $this->bulletin_id);
				$ret .= $temp ;
	...
	function aff_suite() {
		...
		$ret_index = "";
		// Catégories
		if ($this->categories_toutes) $ret_index .= "<tr class='tr_categ'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg['categories_start']."</span></td><td class='public_line_value'>".$this->categories_toutes."</td></tr>";

		// Concepts
		$concepts_list = new skos_concepts_list();
		if ($concepts_list->set_concepts_from_object(TYPE_NOTICE, $this->notice_id)) {
			$ret_index .= "<tr class='tr_concept'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg['concepts_start']."</span></td><td class='public_line_value'>".skos_view_concepts::get_list_in_notice($concepts_list)."</td></tr>";
		}
		...
		if($mots_cles) $ret_index.= "<tr class='tr_keywords'><td align='right' class='bg-grey'><span class='etiq_champ'>".$libelle_key."</span></td><td class='public_line_value'>".nl2br($mots_cles)."</td></tr>";

		// indexation interne
		if($this->notice->indexint) {
			$indexint = new indexint($this->notice->indexint);
			$ret_index.= "<tr class='tr_indexint'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg['indexint_start']."</span></td><td class='public_line_value'>".inslink($indexint->name,  str_replace("!!id!!", $this->notice->indexint, $this->lien_rech_indexint))." <span>".nl2br(htmlentities($indexint->comment,ENT_QUOTES, $charset))."</span></td></tr>" ;
		}
		if ($ret_index) {
			$ret.=$ret_index;
			//$ret.="<tr class='tr_spacer'><td colspan='2' class='td_spacer'>&nbsp;</td></tr>";
		...
		// résumé
		if($this->notice->n_resume) $ret .= "<tr class='tr_nresume'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg['n_resume_start']."</span></td><td class='td_resume public_line_value'>".nl2br($this->notice->n_resume)."</td></tr>";

		// note de contenu
		if($this->notice->n_contenu) $ret .= "<tr class='tr_contenu'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg['n_contenu_start']."</span></td><td class='public_line_value'>".nl2br(htmlentities($this->notice->n_contenu,ENT_QUOTES, $charset))."</td></tr>";
		...
		if (!$this->p_perso->no_special_fields) {
			...
			for ($i=0; $i<count($this->memo_perso_["FIELDS"]); $i++) {
				...
				if ($p['OPAC_SHOW'] && $p["AFF"]) $perso_aff .="<tr class='tr_persofield'><td align='right' class='bg-grey'><span class='etiq_champ'>".strip_tags($p["TITRE"])."</span></td><td class='public_line_value'>".nl2br($p["AFF"])."</td></tr>";
			}
		}
		$ret .= $perso_aff ;
		...
		if ($this->notice->lien) {
			//$ret.="<tr class='tr_spacer'><td colspan='2' class='td_spacer'>&nbsp;</td></tr>";
			$ret.="<tr class='tr_lien'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg["lien_start"]."</span></td><td class='public_line_value'>" ;
			...
			$ret.="</td></tr>";
			if ($this->notice->eformat && substr($this->notice->eformat,0,3)!='RSS') $ret.="<tr class='tr_eformat'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg["eformat_start"]."</span></td><td class='public_line_value'>".htmlentities($this->notice->eformat,ENT_QUOTES,$charset)."</td></tr>";
		}
		// Permalink avec Id
		if ($opac_permalink) {
			if($this->notice->niveau_biblio != "b"){
				$ret.= "<tr class='tr_permalink'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg["notice_permalink"]."</span></td><td class='public_line_value'><a href='".$opac_url_base."index.php?lvl=notice_display&id=".$this->notice_id."'>".substr($opac_url_base."index.php?lvl=notice_display&id=".$this->notice_id,0,80)."</a></td></tr>";
			}else {
				$ret.= "<tr class='tr_permalink'><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg["notice_permalink"]."</span></td><td class='public_line_value'><a href='".$opac_url_base."index.php?lvl=bulletin_display&id=".$this->bulletin_id."'>".substr($opac_url_base."index.php?lvl=bulletin_display&id=".$this->bulletin_id,0,80)."</a></td></tr>";
			}
		}
	...
	public static function expl_list($type,$id,$bull_id=0,$build_ifempty=1) {
		...
		while(($expl = pmb_mysql_fetch_object($res))) {
			...
			for ($i=0; $i<count($colonnesarray); $i++) {
				...
				} else if ($colonnesarray[$i]=="expl_comment") {
					$expl_liste.="<td class='".$colonnesarray[$i]."'>".nl2br(htmlentities($colencours,ENT_QUOTES, $charset))."</td>";
				} else {
	...
	function do_image(&$entree,$depliable) {
		...
		if ($this->notice->code || $this->notice->thumbnail_url) {
			if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $this->notice->thumbnail_url)) {
				...
				if ($depliable) {
					$image = "<img class='vignetteimg' src='".$opac_url_base."images/vide.png' title=\"".$title_image_ok."\" align='right' hspace='4' vspace='2' isbn='".$code_chiffre."' url_image='".$url_image."' vigurl=\"".$this->notice->thumbnail_url."\" alt='".$msg["opac_notice_vignette_alt"]."' />";
				} else {
					if ($this->notice->thumbnail_url) {
	...
	function get_bulletins_docnums() {
		global $dbh, $opac_show_links_invisible_docnums;
		...
		$sql_explnum = "SELECT explnum_id, explnum_nom, explnum_nomfichier, explnum_url, explnum_mimetype 
							...
							WHERE bulletin_notice = ".$this->notice_id." order by explnum_id";
		$explnums = pmb_mysql_query($sql_explnum);
		$explnumscount = pmb_mysql_num_rows($explnums);
	...
	protected function do_connectors() {
		...
		if (pmb_mysql_num_rows($result)) {
			...
			if ($external_rec_id['connector'] == 'cairn') {
				...
				$this->notice->lien.= $cairn_connector->get_sso_params();
			}
		}
	}
	
	function get_pdf_css_style() {
		$css_style = "
			<style type='text/css'>
				td.bg-grey {
					width:156;
