...
// +-------------------------------------------------+
// $Id: translation.class.php,v 1.2.4.1 2017-04-21 09:27:15 ngantier Exp $
...
class translation {
	...
	public static function get_text($id, $trans_table, $trans_field, $text="", $mylang="") {
		...
		$req = "SELECT trans_text FROM translation WHERE trans_table='".$trans_table."' and trans_field='$trans_field' and trans_num='".$id."' and trans_lang='".$mylang."' ";
		$myQuery = pmb_mysql_query($req);
		if(pmb_mysql_num_rows($myQuery)) {
			$myreq = pmb_mysql_fetch_object($myQuery);
			if($myreq->trans_text) {
				return($myreq->trans_text);
			}
		}
		return $text;
	}
