...
// +-------------------------------------------------+
// $Id: authority.class.php,v 1.3.2.3 2017-03-20 14:26:59 ngantier Exp $
...
class authority {
	...
	public function __construct($type_autority,$id){
		$this->type_object = $type_autority; // report aff p_perso depuis la dev
	...
	public function get_p_perso() {
		if (!$this->p_perso) {
			$this->p_perso = array();
			if($this->get_type_object() != "concept"){
				$parametres_perso = new parametres_perso($this->get_prefix_for_pperso());
				$ppersos = $parametres_perso->show_fields($this->get_id());
