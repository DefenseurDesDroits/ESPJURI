...
// +-------------------------------------------------+
// $Id: zip.class.php,v 1.1.4.3 2017-06-01 12:48:01 arenou Exp $
	...
	function getPagesSizes(){
		...
		if(!$this->pagesSizes){
			...
			foreach ($this->entries as $entry) {
				$find = false;
				$content = zip_entry_read($entry,zip_entry_filesize($entry));
				file_put_contents($base_path.'/temp/pmb_zip'.$page, $content);
				exec("exiftool -ImageSize " .$base_path.'/temp/pmb_zip'.$page,$tab);
				if($tab){
					$infos = explode(":",$tab[0]);
					if(trim($infos[0]) == "Image Size"){
						$size = explode("x",trim($infos[1]));
				...
				unlink($base_path.'/temp/pmb_zip'.$page);
				if(!$find){
					$src_img = imagecreatefromstring($content);
					$this->pagesSizes[$page] =array(
							'width' => imagesx($src_img)*1,
							'height'=>  imagesy($src_img)*1
					);
				}
				$page++;
			}
