...
// +-------------------------------------------------+
// $Id: record_display.class.php,v 1.9.2.20 2017-04-26 12:52:18 jpermanne Exp $
...
class record_display {
	...
	static public function do_image($notice_id, &$entree,$depliable) {
		...
		if ($record_datas->get_code() || $record_datas->get_thumbnail_url()) {
			if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $record_datas->get_thumbnail_url())) {
				...
				if (($record_datas->get_thumbnail_url()) && (preg_match('`^(.+)?getimage\.php`',$record_datas->get_thumbnail_url(),$out))) {
					if (trim($out[1])) {
						$url_image = $record_datas->get_thumbnail_url();
					} else {
						$url_image = $opac_url_base.$record_datas->get_thumbnail_url();
					}
				} else {
					$url_image = $opac_url_base."getimage.php?url_image=".urlencode($opac_book_pics_url)."&noticecode=!!noticecode!!&vigurl=".urlencode($record_datas->get_thumbnail_url()) ;
				}
				$title_image_ok = "";
				...
				$ret .= "
					<td class='avis_detail_commentaire_".$data['note']."'>
						<span class='avis_entete'>
							<b>".htmlentities($data['sujet'],ENT_QUOTES,$charset)."</b>, ".$data['date_avis'];
	...
	static public function get_display_explnums($notice_id) {
		...
		if ($record_datas->get_niveau_biblio() == "b" && ($explnums = show_explnum_per_notice(0, $bulletin_id, ''))) {
			return "<a name='docnum'><h3><span id='titre_explnum'>".$msg['explnum']."</span></h3></a>".$explnums;
		}
	...
	static public function get_display_serialcirc_form_actions($notice_id) {
		...
		if($_SESSION['id_empr_session'] && $opac_serialcirc_active && $record_datas->get_opac_serialcirc_demande() && $allow_serialcirc) {
			if($record_datas->get_niveau_biblio() == "s"){
				// pour un pério, on affiche un bouton pour demander l'inscription à une liste de diffusion
