...
// +-------------------------------------------------+
// $Id: facette_search.class.php,v 1.55.2.20 2017-04-27 09:40:05 jpermanne Exp $
...
class facettes{
	...
	public function facettes($objects_ids = ''){
	...
	protected function facette_existing(){
		...
		$this->facettes = array();
		$query = "SELECT * FROM facettes WHERE facette_visible=1 ORDER BY facette_order, facette_name";
		$result = pmb_mysql_query($query);
		while($row = pmb_mysql_fetch_object($result)){
			if($opac_view_filter_class) {
				if(!$opac_view_filter_class->is_selected("facettes", $row->id_facette+0))  continue;
			}
			$this->facettes[] = array(
				'id'=> $row->id_facette+0,
		...
		if($this->objects_ids != ""){
			foreach ($this->facettes as $facette) {
				$query = "select value ,count(distinct id_notice) as nb_result from (SELECT value,id_notice FROM notices_fields_global_index
					...
					AND code_champ = ".($facette['id_critere']+0)."
					AND code_ss_champ = ".($facette['id_ss_critere']+0)."
	...
	public static function see_more($json_facette_plus){
		...
		for($j=0;$j<$count;$j++){
			...
			$tmpArray['facette_value'] = encoding_normalize::json_encode(array($facette_opac['name'],$facette_opac['value'][$j],$facette_opac['code_champ'],$facette_opac['code_ss_champ'],$tmpArray['facette_id'],$facette_opac['nb_result'][$j]));
	...
	public static function get_nb_facettes() {
		$query = "SELECT count(id_facette) FROM facettes WHERE facette_visible=1";
		$result = pmb_mysql_query($query);
	...
	public static function call_ajax_facettes() {
		global $base_path;
		$ajax_facettes = "";
		if(facettes::get_nb_facettes()) {
			$ajax_facettes .= self::get_facette_wrapper();
			$ajax_facettes .="
				<div id='facette_wrapper'>
					<img src='".get_url_icon('patience.gif')."'/>
					<script type='text/javascript'>
						var req = new http_request();
						req.request(\"".$base_path."/ajax.php?module=ajax&categ=facette&sub=call_facettes\",false,null,true,function(data){
	...
	public static function make_facette($objects_ids){
		$return = "";
		$facettes = new facettes($objects_ids);
	...
	public static function make_ajax_facette($objects_ids){
		$facettes = new facettes($objects_ids);
		return array(
			'exists_with_results' => ($_SESSION["cms_build_activate"] ? true : $facettes->exists_with_results),
			...
			function facette_see_more(id,json_facette_plus){
				...
				if (json_facette_plus == null) {
					...
					if (see_more_less.getAttribute('etat')=='plus') {
						see_more_less.innerHTML='".$msg["facette_moins_link"]."';
						see_more_less.setAttribute('etat','moins');
					} else {
						see_more_less.innerHTML='".$msg["facette_plus_link"]."';
					...
					req.request(\"./ajax.php?module=ajax&categ=facette&sub=see_more\",true,'sended_datas='+encodeURIComponent(JSON.stringify(sended_datas)),true,function(data){
						...
						ahref.innerHTML='".$msg["facette_moins_link"]."';
						myTable.appendChild(tr);
					});
				}
			}
	...
	public function create_ajax_table_facettes(){
			...
			foreach ($vTabFacette['value'] as $keyValue=>$vLibelle) {
				...
				if (!$clicked) {
					...
					if ($affiche) {
						...
						$facette_plus[$this->facettes[$keyFacette]['id']]['facette'][]=$vLibelle." "."(".$vTabFacette['nb_result'][$keyValue].")";
						$facette_plus[$this->facettes[$keyFacette]['id']]['value'][]=$vLibelle;	
						$facette_plus[$this->facettes[$keyFacette]['id']]['nb_result'][]=$vTabFacette['nb_result'][$keyValue];
						$facette_plus[$this->facettes[$keyFacette]['id']]['code_champ']=$vTabFacette['code_champ'];
						$facette_plus[$this->facettes[$keyFacette]['id']]['code_ss_champ']=$vTabFacette['code_ss_champ'];
						$facette_plus[$this->facettes[$keyFacette]['id']]['name']=$vTabFacette['name'];
		...
		if (is_array($_SESSION['facette']) && count($_SESSION['facette'])) {
					...
					if (!isset($detailFacette['see_more'])) {
						...
						$table_facette .= "
							<tr class='facette_tr_see_more'>
								<td colspan='3'>
									<a href='javascript:facette_see_more(".$idfacette.",".encoding_normalize::json_encode($facette_plus[$idfacette]).");'>".$msg["facette_plus_link"]."</a>
								</td>
							</tr>";
						$flagSeeMore = true;
					}
		...
		if($opac_compare_notice_active*1=="1"){
			...
			if(count($facette_compare->facette_compare)){
				...
				<td><a href='$base_path/index.php?lvl=more_results&get_last_query=1&reinit_compare=1' class='facette_compare_raz'><img alt='".htmlentities($msg['facette_compare_reinit'],ENT_QUOTES,$charset)."' title='".htmlentities($msg['facette_compare_reinit'],ENT_QUOTES,$charset)."' width='18px' height='18px' src='".get_url_icon('cross.png')."'/></a></td></tr></table>
		...
		foreach($notices_list as $notice_id){		
			...
			if($r=pmb_mysql_fetch_object($res)){
				...
				if (substr($opac_notice_reduit_format_similaire,0,1)!="H" && $opac_show_book_pics=='1') {
					...
					if ($r->thumbnail_url) {
						if(strpos($r->thumbnail_url,'data:image',0)===0) {
							$url_image_ok = $r->thumbnail_url;
						} else {
							$url_image_ok=$opac_url_base.'getimage.php?url_image='.rawurlencode($r->thumbnail_url);
						}
						$title_image_ok="";
