...
// +-------------------------------------------------+
// $Id: extended.inc.php,v 1.87.2.8 2016-12-01 15:34:02 apetithomme Exp $
...
if(!$opac_allow_affiliate_search || ($opac_allow_affiliate_search && $tab == "catalog")){
	...
	if ($count) {
		...
		if ($count<=$opac_nb_max_tri) {
			...
			if ($_SESSION["last_sortnotices"]!="" || $opac_default_sort_display) {
				$sr_form.= '<span class="triContainer">';
			}
			$sr_form.= $affich_tris_result_liste;
			
			if ($_SESSION["last_sortnotices"]!="") {
				$sort=new sort('notices','session');
				$sr_form.= "<span class='sort'>".$msg['tri_par']." ".$sort->descriptionTriParId($_SESSION["last_sortnotices"])."<span class=\"espaceResultSearch\">&nbsp;</span></span>";
				$sr_form.= '</span>';
			} elseif ($opac_default_sort_display) {
				$sort=new sort('notices','session');
				$sr_form.= "<span class='sort'>".$msg['tri_par']." ".$sort->descriptionTriParId("default")."<span class=\"espaceResultSearch\">&nbsp;</span></span>";
				$sr_form.= '</span>';
			}
		} else {
