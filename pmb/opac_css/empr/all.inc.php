...
// +-------------------------------------------------+
// $Id: all.inc.php,v 1.46.2.16 2017-06-02 12:05:23 dgoron Exp $
...
if ($nb_elements) {
	if (!$dest) {
		...
		while ($data = pmb_mysql_fetch_array($req)) { 
			...
			echo "<td column_name='".htmlentities($msg["date_loan"],ENT_QUOTES,$charset)."'><center>".$data['aff_pret_date']."</center></td>"; 
				
			if ($data['retard']) echo "<td class='expl-empr-retard' column_name='".htmlentities($msg["date_back"],ENT_QUOTES,$charset)."'><center>".$data['aff_pret_retour']."</center></td>";
				else echo "<td column_name='".htmlentities($msg["date_back"],ENT_QUOTES,$charset)."'><center>".$data['aff_pret_retour']."</center></td>";
			...
			if ($opac_pret_prolongation==1 && $allow_prol) {
				...
				echo "<td column_name='".htmlentities($msg["opac_titre_champ_nb_prolongation"],ENT_QUOTES,$charset)."'><center>".$nb_prolongation."/".$pret_nombre_prolongation."</center></td>";
				...
				if ($prolongation) {
					echo "<td column_name='".htmlentities($msg["opac_titre_champ_prolongation"],ENT_QUOTES,$charset)."'><center><a href='./empr.php?prolongation=$aff_date_prolongation&prolonge_id=$expl_id&tab=loan_reza&lvl=$lvl#empr-loan' $js >$aff_date_prolongation</a></center></td>";
				} else {
					echo "<td column_name='".htmlentities($msg["opac_titre_champ_prolongation"],ENT_QUOTES,$charset)."' style='cursor: default' $js ><center><img src='".get_url_icon("no_prolongation.png")."' border='0' title='".htmlentities($no_prolong_explanation,ENT_QUOTES,$charset)."' /></center></td>";
				}
		
			} // fin if prolongeable	
			/* test de date de retour dépassée */
			if ($lvl!="late")
				if ($data['retard']) echo "<td class='expl-empr-retard' column_name='".htmlentities($msg["empr_late"],ENT_QUOTES,$charset)."'><center><b>&times;</b></center></td>";
		...
		if ($nb_elements) {	
			if (!$dest) {
				...
				while ($data = pmb_mysql_fetch_array($req)) {
					...
					if ($data['retard']) echo "<td column_name='".htmlentities($msg["date_back"],ENT_QUOTES,$charset)."' class='expl-empr-retard'><center>".$data['aff_pret_retour']."</center></td>";
						else echo "<td column_name='".htmlentities($msg["date_back"],ENT_QUOTES,$charset)."'><center>".$data['aff_pret_retour']."</center></td>";
						/* test de date de retour dépassée */
					if ($lvl!="late")
						if ($data['retard']) echo "<td column_name='".htmlentities($msg["empr_late"],ENT_QUOTES,$charset)."' class='expl-empr-retard'><center><b>&times;</b></center></td>";
						else echo "<td column_name='".htmlentities($msg["empr_late"],ENT_QUOTES,$charset)."'>&nbsp;</td>";
					echo "</tr>\n";
