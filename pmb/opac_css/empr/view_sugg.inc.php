...
// +-------------------------------------------------+
// $Id: view_sugg.inc.php,v 1.12.4.2 2016-06-28 08:45:41 jpermanne Exp $
...
if($nbr){
	for($i=0;$i<$nbr;$i++) {
		...
		if($sug->get_explnum('id')){
			$sug_form .="<td align='center'><i>".($sug->get_explnum('id') ? "<a href=\"$base_path/explnum_doc.php?explnumdoc_id=".$sug->get_explnum('id')."\" target=\"_blank\"><img src='".get_url_icon("globe_orange.png")."' border='0' /></a>" : '' )."</i></td>";
		} else {
			$sug_form .="<td></td>";
		}
		$sug_form.= "
