...
// +-------------------------------------------------+
// $Id: cms_module_recordslist_view_carousel_responsive.class.php,v 1.4.4.3 2016-09-21 07:58:52 apetithomme Exp $
...
class cms_module_recordslist_view_carousel_responsive extends cms_module_common_view_carousel_responsive {
	...
	public function render($records){
		...
		if(pmb_mysql_num_rows($result)){
			while($result && $row = pmb_mysql_fetch_object($result)){
				if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $row->thumbnail_url)) {
					...
					if ($row->thumbnail_url){
						if(strpos($row->thumbnail_url,'data:image',0)===0) {
							$url_vign = $row->thumbnail_url;
						} else {
							$url_vign=$opac_url_base.'getimage.php?url_image='.rawurlencode($row->thumbnail_url);
						}
