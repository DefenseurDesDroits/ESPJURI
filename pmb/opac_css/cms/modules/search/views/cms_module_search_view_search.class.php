...
// +-------------------------------------------------+
// $Id: cms_module_search_view_search.class.php,v 1.23.2.5 2016-09-21 07:58:51 apetithomme Exp $
...
class cms_module_search_view_search extends cms_module_common_view{
	...
	public function render($datas){
		...
		if ($this->parameters['link_search_advanced']) {
			$html.="
				<p class='search_advanced_link'><a href='./index.php?search_type_asked=simple_search".($this->parameters['link_to_opac_view'] ? '&opac_view='.$this->parameters['link_to_opac_view'] : '')."'>".$this->format_text($this->msg['cms_module_search_view_link_search_advanced_display'])."</a></p>";
		}
		if(is_array($this->parameters['others_links']) && count($this->parameters['others_links'])) {
			foreach ($this->parameters['others_links'] as $key=>$other_link) {
				$html.="
