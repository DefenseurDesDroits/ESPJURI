...
// +-------------------------------------------------+
// $Id: cms_module_metadatas_datasource_metadatas_record.class.php,v 1.1.4.2 2016-08-26 10:07:41 dbellamy Exp $
			...
			if($notice){
				...
				if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $notice_class->thumbnail_url)) {
					...
					if ($notice_class->thumbnail_url){
						if(strpos($notice_class->thumbnail_url,'data:image',0)===0) {
							$url_vign = $notice_class->thumbnail_url;
						} else {
							$url_vign=$opac_url_base.'getimage.php?url_image='.rawurlencode($notice_class->thumbnail_url);
						}
