...
// +-------------------------------------------------+
// $Id: cms_module_common_view_recordslist.class.php,v 1.10.4.2 2016-08-26 10:07:40 dbellamy Exp $
			...
			foreach($datas["records"] as $notice){
				...
				if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $notice_class->notice->thumbnail_url)) {
					...
					if ($notice_class->notice->thumbnail_url){
						if(strpos($notice_class->notice->thumbnail_url,'data:image',0)===0) {
							$url_vign = $notice_class->notice->thumbnail_url;
						} else {
							$url_vign=$opac_url_base.'getimage.php?url_image='.rawurlencode($notice_class->notice->thumbnail_url);
						}
