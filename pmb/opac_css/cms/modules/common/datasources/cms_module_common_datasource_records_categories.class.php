...
// +-------------------------------------------------+
// $Id: cms_module_common_datasource_records_categories.class.php,v 1.9.2.1 2016-09-20 15:09:54 vtouchard Exp $
...
class cms_module_common_datasource_records_categories extends cms_module_common_datasource_list{
	...
	public function get_datas(){
		...
		if ($selector) {
			...
			JOIN cms_articles_descriptors ON cms_articles_descriptors.num_noeud = notices_categories.num_noeud
			AND num_article ='".($selector->get_value()*1)."' where notice_id in(".implode(',', $return['records']).")group by notice_id order by count(*) desc, create_date desc";
