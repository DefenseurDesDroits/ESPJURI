...
// +-------------------------------------------------+
// $Id: resa_func.inc.php,v 1.39.2.2 2016-05-02 09:55:48 jpermanne Exp $
		...
		while ($empr=@pmb_mysql_fetch_object($result)) {
			...
			if (!$output_final) {
				...
				if ($annul==1) {
					...
					$output_final .= "<a href='".$opac_url_base."'><font color=red><strong>".$sujet ;					
				} elseif ($annul==2) {
					if ($resa_planning) {
					...
					$output_final .= "<a href='".$opac_url_base."'><font color=blue><strong>".$sujet ;					
				} else {
					if ($resa_planning) {
					...
					$output_final .= "<a href='".$opac_url_base."'><font color=green><strong>".$sujet ;					
				}
