...
// +-------------------------------------------------+
// $Id: notice_tpl.inc.php,v 1.78.2.29 2017-05-12 09:33:15 jpermanne Exp $
...
function aff_vignette($param) {
	global $parser_environnement;
	global $opac_url_base,$opac_book_pics_url;
	global $charset;
	...
	if ($notice['notice_info']->notice->code || $notice['notice_info']->notice->thumbnail_url) {
		$code_chiffre = pmb_preg_replace('/-|\.| /', '', $notice['notice_info']->notice->code);
		$url_image="";
		//On évite de boucler pour les images proxifiées
		if (($notice['notice_info']->notice->thumbnail_url) && (preg_match('`^(.+)?getimage\.php`',$notice['notice_info']->notice->thumbnail_url,$out))) {
			if (trim($out[1])) {
				$url_image = $notice['notice_info']->notice->thumbnail_url;
			} else {
				$url_image = $opac_url_base.$notice['notice_info']->notice->thumbnail_url;
			}
		} else {
			$url_image = $opac_url_base."getimage.php?url_image=".urlencode($opac_book_pics_url)."&noticecode=!!noticecode!!&vigurl=".urlencode($notice['notice_info']->notice->thumbnail_url) ;
		}
		
		if ($notice['notice_info']->notice->thumbnail_url) {
			$url_image_ok = $url_image;
			if(strpos($notice['notice_info']->notice->thumbnail_url,'data:image',0)===0) {
				$url_image_ok = $notice['notice_info']->notice->thumbnail_url;
			}
		} else {
			$url_image_ok = str_replace("!!noticecode!!", $code_chiffre, $url_image) ;
		}
		//Si l'url finale n'est pas un data:image, on encode bien l'url
		if(strpos($url_image_ok,'data:image')===false) {
			$url_image_ok = htmlspecialchars($url_image_ok,ENT_NOQUOTES,$charset);
		}
		$image = "<img class='tpl_vignette' src='".$url_image_ok."' />";
	}
