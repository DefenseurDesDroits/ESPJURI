...
// +-------------------------------------------------+
// $Id: index_includes.inc.php,v 1.103.2.21 2017-06-08 08:44:23 jpermanne Exp $
...
if ($time_expired==1) {
	...
	echo "<script>alert(\"".sprintf($msg["anonymous_session_expired"],round($opac_duration_session_auth/60))."\");</script>";
}

//from_permalink va permettre de stocker la recherche en session même si autolevel2 = 0
if($opac_autolevel2 || $from_permalink){
	$es=new search();
}
require_once($base_path.'/includes/navigator.inc.php');

$link_to_print_search_result = "<span class=\"printSearchResult\">
<a href='#' onClick=\"openPopUp('".$base_path."/print.php?lvl=search&current_search=".($_SESSION['last_query']+0)."','print',500,600,-2,-2,'scrollbars=yes,menubar=0'); w.focus(); return false;\" title=\"".$msg["histo_print"]."\">
	<img src='".get_url_icon('print.gif')."' border='0' align='bottom' alt=\"".$msg["histo_print"]."\"/>
</a>
</span>";
...
if ($opac_show_bandeaugauche==0) {
	...
	if (!$_SESSION["user_code"]) {
		...
		if($opac_quick_access) {
			...
			if ($allow_book && $opac_resa && $opac_resa_planning) {
				$loginform__.= '<option value="empr.php?tab=loan_reza&lvl=all#empr-resa_planning">'.$msg['empr_my_resas_planning'].'</option>';
			}
