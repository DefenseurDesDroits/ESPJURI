...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.51.2.14 2017-03-24 10:21:10 jpermanne Exp $
...
function aff_query_auth_empr_search($field,&$check_scripts,$varname) {
	...
	$ret.="
		...
		<span class='search_value_sub'><input autfield='$fname_id'  onkeyup='fieldChanged(\"$fnamesans\",this.value,event);' callback='authoritySelected' completion='$completion' $fnamevar_id id='$fnamesanslib' name='$fnamelib' value='".htmlentities($libelle,ENT_QUOTES,$charset)."' type='text' class='ext_search_txt' /></span>
...
function chk_date_inter_empr($field,&$check_message) {
	...
	if (is_array($val)) {
		foreach($val as $interval){
			if($interval['date_begin'] && $interval['time_begin'] && $interval['date_end'] && $interval['time_end']) {
