...
// +-------------------------------------------------+
// $Id: etagere_see.inc.php,v 1.43.2.7 2017-03-03 15:44:34 jpermanne Exp $
...
if ($id) {
	...
	$id+=0;
	$requete="select idetagere,name,comment,id_tri, thumbnail_url from etagere where idetagere=$id";
	$resultat=pmb_mysql_query($requete);
	$r=pmb_mysql_fetch_object($resultat);

	print pmb_bidi(($r->thumbnail_url?"<img src='".$r->thumbnail_url."' border='0' class='thumbnail_etagere' alt='".$r->name."'>":"")."<h3><span>".$r->name."</span></h3>\n");
	...
	if($nbr_lignes) {
		...
		if ($nbr_lignes>$opac_nb_max_tri) {
			...
			if ($_SESSION["last_sortnotices"]!="") {
				...
				print "<span class='sort'>".$msg['tri_par']." ".$sort_class->descriptionTriParId($_SESSION["last_sortnotices"])."<span class=\"espaceResultSearch\">&nbsp;</span></span>"; 
			} 
		}
		//fin gestion du tri
