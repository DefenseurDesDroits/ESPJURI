...
// +-------------------------------------------------+
// $Id: empr_includes.inc.php,v 1.95.2.15 2017-01-18 08:42:45 ngantier Exp $
...
if (($opac_show_login_form_next) && ($login) && ($first_log) && ($lvl!='change_password')) die ("<script type='text/javascript'>document.location='$opac_show_login_form_next';</script>");
		...
		switch($tab){
			...
			case 'dsi':
				...
				if ($opac_allow_bannette_priv && $allow_dsi_priv){
					...
					if ($opac_allow_bannette_priv == 2) {
						$link_alert = './index.php?tab=dsi&bt_cree_bannette_priv=1&search_type_asked=simple_search';
					}
