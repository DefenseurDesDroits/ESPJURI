...
// +-------------------------------------------------+
// $Id: bannette_func.inc.php,v 1.37.4.10 2017-05-17 08:28:49 jpermanne Exp $
...
function affiche_bannette($bannettes="", $aff_notices_nb=0, $mode_aff_notice=AFF_BAN_NOTICES_BOTH, $depliable=AFF_BAN_NOTICES_DEPLIABLES_OUI, $link_to_bannette="", $liens_opac=array(), $date_diff='', $htmldiv_id="bannette-container", $htmldiv_class="bannette-container", $htmldiv_zindex="",$home=false ) {
	...
	for ($i=0; $i<sizeof($tableau_bannettes); $i++ ) {
		...
		if ($recherche) {
			$aff_banette.= "<a id=\"bannette_human_query_link".$id_bannette."\" href=\"#\" onclick=\"javascript:document.getElementById('bannette_human_query".$id_bannette."').style.display='';document.getElementById('bannette_human_query_link".$id_bannette."').style.display='none';\">".$msg['bannette_human_query_show']."</a>";
			$aff_banette.= "<div id='bannette_human_query".$id_bannette."' class='bannette_human_query' style='display:none'>";
			$aff_banette.= $recherche;
			$aff_banette.= "</div>";
		}
			...
			if ($notice->visu_notice) {
				...
				switch ($mode_aff_notice) {
					...
					case AFF_BAN_NOTICES_TEMPLATE_DJANGO :
						...
						$aff_banette .= record_display::get_display_in_result($idnotice);
						break;
					default:
						$notice->do_isbd();
						$notice->do_public();		
		...
		if ($limite_notices&&($limite_notices<count($notices))){
			if ($link_to_bannette) $aff_banette.="<a href=\"".str_replace("!!id_bannette!!",$id_bannette,$link_to_bannette)."\"><span class='banette-suite'>".$msg['banette_suite']."</span></a>";
		}
...
function tableau_gerer_bannette($priv_pub="PUB") {
	...
	//Récupération des infos des bannettes
	if ($priv_pub=="PUB") {
		$rqt = "select groupe_id from empr_groupe where empr_id=".$id_empr." AND groupe_id != 0";//En création de lecteur une entrée avec groupe_id = 0 est créée ...
...
function gerer_abon_bannette( $priv_pub="PUB", $link_to_bannette="", $htmldiv_id="bannette-container", $htmldiv_class="bannette-container", $htmldiv_zindex="" ) {
	...
	foreach ($tableau_bannettes as $v) {
		if ($v['abonn']=='checked') {
			$tableau_bannettes_abo_checked[] = $v['id_bannette'];
		}
	}
	...
	for ($i=0; $i<sizeof($tableau_bannettes); $i++ ) {
		...
		$comment_public=$tableau_bannettes[$i]['comment_public'] ;
		$aff_date_last_envoi=$tableau_bannettes[$i]['aff_date_last_envoi'] ;
		$retour_aff.="\n<tr><td column_name='".htmlentities($msg['dsi_bannette_gerer_abonn'],ENT_QUOTES,$charset)."' align='right' valign='top'>";
		...
		$retour_aff.="\n</td><td column_name='".htmlentities($msg['dsi_bannette_gerer_nom_liste'],ENT_QUOTES,$charset)."' align='left' valign='top'>";
		...
		$retour_aff.="\n</td><td column_name='".htmlentities($msg['dsi_bannette_gerer_date'],ENT_QUOTES,$charset)."' align='center' valign='top'>";
		$retour_aff.= htmlentities($aff_date_last_envoi,ENT_QUOTES, $charset);
		$retour_aff.="\n</td><td column_name='".htmlentities($msg['dsi_bannette_gerer_nb_notices'],ENT_QUOTES,$charset)."' align='center' valign='top'>";
		$retour_aff.= htmlentities($tableau_bannettes[$i]['nb_contenu'],ENT_QUOTES, $charset);
		$retour_aff.="\n</td><td column_name='".htmlentities($msg['dsi_bannette_gerer_periodicite'],ENT_QUOTES,$charset)."' align='center' valign='top'>";
