...
// +-------------------------------------------------+
// $Id: messages.class.php,v 1.2.10.1 2017-05-31 12:42:39 arenou Exp $
...
class message {
 ...
 // Modif Armelle Nedelec recherche de l'encodage du fichier xml et transformation en charset'
 	function analyser() 
 	{
 		global $charset,$visionneuse_path;

 		$fileInfo = pathinfo($this->fichierXml);
 		$tempFile = $visionneuse_path."/temp/XML".preg_replace("/[^a-z0-9]/i","",$fileInfo['dirname'].$fileInfo['filename'].$charset).".tmp";

 		$dejaParse = false;
 		if (file_exists($tempFile) ) {
 			//Le fichier XML original a-t-il été modifié ultérieurement ?
 			if (filemtime($this->fichierXml) > filemtime($tempFile) || filemtime($this->fichierXmlSubst) > filemtime($tempFile)) {
