...
// +--------------------------------------------------------------------------+
// $Id: print.php,v 1.73.2.11 2017-05-23 08:37:37 jpermanne Exp $
...
if ($action!="print_$lvl") {
	...
	$output_final .= "<div id='mail_part'>
		<blockquote>
			".$msg["print_emaildest"]."&nbsp;<input type='text' size='30' name='emaildest' value='' /><br /><br />
			".$msg["print_emailexp"]."&nbsp;<input type='text' size='30' name='emailexp' value='' /><br />
			".$msg["print_emailobj_label"]."&nbsp;<input type='text' size='30' name='emailobj' value='".trim($msg["print_emailobj"]." ".$opac_biblio_name." - ".formatdate(today()))."' /><br />
	...
	if (count($notices)) {
		...
		for ($i=0; $i<count($notices); $i++) {
			...
			if($noti_tpl) {
				...
				$current->do_header();
				if (!$has_css_for_pdf) {
					$notices_aff .= $current->get_pdf_css_style();
					$has_css_for_pdf = true;
				}
				if ($type=='PUBLIC') {
...
if ($output!='email') {
	...
	if (!$emailobj) {
		$emailobj=$msg['print_emailobj'].' '.$opac_biblio_name.' - '.$date_today;
	}
	$res_envoi=mailpmb('', $emaildest, $emailobj, $output_final.'<br /><br />'.mail_bloc_adresse().'</body></html>', $opac_biblio_name, $opac_biblio_email, $headers);
	$vide_cache=filemtime("./styles/".$css."/".$css.".css");
	if ($res_envoi) { 
		print "<html><head><meta http-equiv=Content-Type content=\"text/html; charset=".$charset."\" /><title>".$msg["print_title"]."</title></head><body class='popup'><link rel=\"stylesheet\" href=\"./styles/".$css."/$css.css?".$vide_cache."\" />\n<br /><br /><center><h3>".sprintf($msg["print_emailsucceed"],$emaildest)."</h3><br />
		<a href=\"\" onClick=\"self.close(); return false;\">".$msg["print_emailclose"]."</a></center></body></html>" ;
	} else { 
		echo "<html><head><meta http-equiv=Content-Type content=\"text/html; charset=".$charset."\" /><title>".$msg["print_title"]."</title></head><body class='popup'><link rel=\"stylesheet\" href=\"./styles/".$css."/$css.css?".$vide_cache."\" />\n<br /><br /><center><h3>".sprintf($msg["print_emailfailed"],$emaildest)."</h3><br />
		<a href=\"\" onClick=\"self.close(); return false;\">".$msg["print_emailclose"]."</a></center></body></html>" ;
	}
}		
