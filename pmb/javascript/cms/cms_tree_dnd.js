...
function cms_compare_forms(){
	if(!document.getElementById('content_infos').querySelector('form')) return false;
...
function cms_register_form(){
	if(document.getElementById('content_infos').querySelector('form')){
...
function cms_load_content_infos(item,node,evt){
	...
	if(item.id != "root"){
		...
		content.onDownloadEnd = function(){
			if(dijit.byId('editorial_type_content_form')){
				dijit.byId('editorial_type_content_form').onDownloadEnd = function(){
					cms_register_form();
				}	
			}else{
				cms_register_form();
